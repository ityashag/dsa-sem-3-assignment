...
public:
    ...
    void deletion_at_k(node *&root, int k)
    {
        if (root == NULL)
        {
            cout << "NULL\n";
        ...
        {
            root = root->delete_form_avl(root, a[i]);
        }
    }
...
int main()
{
    ...
    cout << "enter the level k for deletion (-1 to exit):";
    cin >> key;

    while (key != -1)
    {

        root->deletion_at_k(root, key);

        cout << "\nalter deletion :level order :\n";
        root->level_order(root);
        cout << "enter the level k (-1 to exit):";
        cin >> key;
    }
}
