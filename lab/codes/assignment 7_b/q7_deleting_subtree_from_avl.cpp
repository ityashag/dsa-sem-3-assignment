...
public:
    ...
    node *delete_subtree_form_avl(node *&root, int key)
    {
        if (root == NULL)
            return root;
        ...
        if (root->data < key)
        {
            root->right = delete_subtree_form_avl(root->right, key);
        }
        else if (root->data > key)
        {
            root->left = delete_subtree_form_avl(root->left, key);
        }
        else
        {
            return NULL;
...
int main()
{
    ...
    cout << "enter the data of first node of sutree for deletion (-1 to exit):";
    cin >> key;

    while (key != -1)
    {

        root = root->delete_subtree_form_avl(root, key);
